...
contract ATokenAerodrome is AToken {
  ...
  function mint(address caller, address onBehalfOf, uint256 amount, uint256 index) external virtual override onlyPool returns (bool ret) {
  ...
  function _claimAndRegisterReward() internal {
    ...
    if (aeroEmissionReceiver != IAerodromeEmissionReceiver(address(0))) {
      aeroEmissionReceiver.receiveAeroEmission(aero.balanceOf(address(this)));
    }
  }
}
